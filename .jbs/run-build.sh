...
if [ -n "" ]; then
  echo "Creating tag 2.1.3 to match enforced version"
  git tag -m 2.1.3 -a 2.1.3 || true
else
  echo "Enforce version not set, recreating original tag version-2.1.3"
  git tag -m version-2.1.3 -a version-2.1.3 || true
fi
